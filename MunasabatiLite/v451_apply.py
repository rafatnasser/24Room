...
def replace_method(signature,new_code):
    s=MAIN.read_text(encoding='utf-8')
    start=s.find(signature)
    if start<0: raise SystemExit('v4.5.1 method not found: '+signature)
    brace=s.find('{',start)
    if brace<0: raise SystemExit('v4.5.1 opening brace not found: '+signature)
    depth=0; end=-1
    ...
    if end<0: raise SystemExit('v4.5.1 closing brace not found: '+signature)
    MAIN.write_text(s[:start]+new_code+s[end:],encoding='utf-8')

def rep(old,new):
    s=MAIN.read_text(encoding='utf-8')
    if old in s: MAIN.write_text(s.replace(old,new),encoding='utf-8')

replace_method('    private void buildUi()',r'''    private void buildUi(){
        ...
        LinearLayout f1=new LinearLayout(this);f1.setPadding(0,dp(7),0,0);searchCard.addView(f1);
        ...
        LinearLayout f2=new LinearLayout(this);f2.setPadding(0,dp(6),0,0);searchCard.addView(f2);dateFilter=outlineButton(tr("كل التواريخ  ▣","All dates  ▣"));f2.addView(dateFilter,new LinearLayout.LayoutParams(0,dp(responsive(38,41,46)),1));dateFilter.setOnClickListener(v->pickFilterDate());Button clear=outlineButton(tr("مسح  ↻","Clear  ↻"));LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(0,dp(responsive(38,41,46)),1);cp.setMargins(dp(7),0,0,0);f2.addView(clear,cp);clear.setOnClickListener(v->{search.setText("");categoryFilter.setSelection(0);dayFilter.setSelection(0);selectedDate=0;dateFilter.setText(tr("كل التواريخ  ▣","All dates  ▣"));render();});
...
replace_method('    private void addBottomNavigation(LinearLayout root)',r'''    private void addBottomNavigation(LinearLayout root){
