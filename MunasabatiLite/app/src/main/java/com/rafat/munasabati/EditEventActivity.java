...
public class EditEventActivity extends Activity {
    ...
    private Spinner reminder, category, recurrence, colorSpinner;
    ...
    private void buildUi(){
        ...
        basic.addView(fieldLabel(tr("لون المناسبة","Event color")));colorSpinner=new Spinner(this);colorSpinner.setAdapter(new ArrayAdapter<>(this,android.R.layout.simple_spinner_dropdown_item,ColorPalette.labels(this,true)));basic.addView(colorSpinner,new LinearLayout.LayoutParams(-1,dp(50)));
        LinearLayout marks=new LinearLayout(this);favoriteBox=new CheckBox(this);favoriteBox.setText(tr("⭐ مفضلة","⭐ Favorite"));pinnedBox=new CheckBox(this);pinnedBox.setText(tr("📌 تثبيت بالأعلى","📌 Pin to top"));marks.addView(favoriteBox,new LinearLayout.LayoutParams(0,-2,1));marks.addView(pinnedBox,new LinearLayout.LayoutParams(0,-2,1));basic.addView(marks);
        ...
        TextView repeatHint=label(tr("«عادة أسبوعية» تضبط أسبوعيًا، و«سنوية» و«عيد ميلاد» و«عيد زواج» تضبط سنويًا تلقائيًا، ويمكنك تغيير الاختيار.","Weekly habit defaults to weekly; Anniversary, Birthday and Wedding anniversary default to yearly. You can change it."),12,false);repeatHint.setTextColor(muted);when.addView(repeatHint);
        category.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener(){public void onItemSelected(android.widget.AdapterView<?> p,View v,int pos,long rowId){if(pos!=lastCategoryPos){lastCategoryPos=pos;String auto=Recurrence.autoForCategory(Categories.CODES[pos]);if(!Recurrence.NONE.equals(auto))recurrence.setSelection(Recurrence.indexOf(auto));}}public void onNothingSelected(android.widget.AdapterView<?> p){}});
